...
router.post("/request/:id", async (req, res) => {
  ...
  const user1Requests = await requests.findOne({
    user1: user,
    user2: req.params.id,
  });
  const user2Requests = await requests.findOne({
    user2: user,
    user1: req.params.id,
  });
...
router.post("/respond/:id", async (req, res) => {
  ...
  if (!user1Requests && !user2Requests) {
    ...
    if (user2Requests["response2"] === STATUS_ACCEPT && doAccept) {
      ...
      const removeRes = await requests.deleteOne({
        user2: user,
        user1: req.params.id,
      });
      if (!removeRes) {
...
router.get("/:id", async (req, res) => {
  const friendRes = await Profile.findOne({ _id: req.params.id });
  if (!friendRes) {
    res.status(400).send("invalid user provided");
  }

  let friendArr = [];
  friendRes["friends"].forEach((friendId) => {
    friendArr.push(Profile.findOne({ _id: friendId }));
  });

  const results = await Promise.all(friendArr);
