...
async function regenerateChallenges(id) {
  await CurrentChallenges.updateOne(
    { user: id },
    {
      challenges: (
...
function buildChallengeRet(challengeId, isComplete) {
  return Challenges.findOne({ _id: challengeId }).then((res) => {
    return {
      challenge: res,
      isComplete,
    };
...
router.post("/complete", async (req, res) => {
  ...
  if (isOverIncrement(Date.now(), userChallenges["generated"], 1)) {
    ...
    if (!regenRes) {
      res
        .status(500)
        .send(
          "Failed to regenerate challenges after expired daily challenges."
        );
      return;
    }
  }
  ...
  if (isWeeklyChallenge) {
    ...
    const updateRes = await CurrentChallenges.findOneAndUpdate(
      { user },
      {
  ...
  const profileUpdateRes = await Profile.findOneAndUpdate(
    { _id: user },
    {
